...
class FilesGenerator:
    ...
    def update_module_manifest(self, module: type[Module]):
        ...
        if missing_fields := self.missing_required_fields(manifest["configuration"]):
            raise ModuleConfigurationError(
                f"Missing required fields {missing_fields} in the Python model"
            )
    ...
    @staticmethod
    def missing_required_fields(manifest_conf: dict[str, Any]) -> list[str]:
        missing_fields: list[str] = []
        for required_field in manifest_conf["required"]:
            if required_field not in manifest_conf["properties"].keys():
        ...
        for field in config_fields:
            if field.field_info.extra.get("secret", False):
                ...
                config["properties"][name] = {
                    "title": FilesGenerator.to_title(name),
                    "type": "string",
                }
    ...
    @staticmethod
    def to_title(field_name):
        """Converts a field name to a title

        Field names are in snake case. We convert them to title case,
        i.e. space between words & words start with capital.

        Parameters
        ----------
        field_name : str
            Field name to be converted to a title
        """
        field_name_as_list = re.sub(r"[-_]", " ", field_name).split()
        title = " ".join(word.capitalize() for word in field_name_as_list)
        return title
