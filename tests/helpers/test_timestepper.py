...
def test_ranges_yields_current_time_range_first(mock_trigger):
    ...
    with (
        patch("sekoia_automation.helpers.timestepper.datetime") as mock_datetime,
        patch("sekoia_automation.helpers.timestepper.time"),
    ):
        mock_datetime.datetime.now.return_value = datetime.datetime(
            2024, 1, 15, 12, 0, 0, tzinfo=datetime.UTC
        )
...
def test_ranges_sleeps_when_next_end_is_in_future(mock_trigger):
    ...
    with (
        patch("sekoia_automation.helpers.timestepper.datetime") as mock_datetime,
        patch("sekoia_automation.helpers.timestepper.time") as mock_time,
    ):
...
def test_create_with_default_parameters(mock_trigger, fixed_time):
    with patch("sekoia_automation.helpers.timestepper.datetime") as mock_datetime:
...
def test_create_from_time_with_custom_parameters(mock_trigger):
    ...
    stepper = TimeStepper.create_from_time(
        mock_trigger, start, frequency=300, timedelta=10
    )
...
def test_ranges_logs_current_lag(mock_trigger):
    ...
    with (
        ...
        log_calls = [
            call
            for call in mock_trigger.log.call_args_list
            if "Current lag" in str(call)
        ]
...
def test_ranges_logs_waiting_message_when_sleeping(mock_trigger):
    ...
    with (
        ...
        log_calls = [
            call for call in mock_trigger.log.call_args_list if "Waiting" in str(call)
        ]
        assert len(log_calls) > 0
